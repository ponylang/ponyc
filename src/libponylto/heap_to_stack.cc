...
// The alloca that replaces a heap allocation inherits its alignment from the
// `align` attribute on pony_alloc/pony_alloc_small. That attribute is set in
// init_runtime_decls (codegen.c) to match the runtime's heap alignment
// guarantee (2^HEAP_MINBITS). Without it the alloca could get a lower
// alignment than the heap would have provided.
