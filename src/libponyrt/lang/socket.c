  ...
  const struct iovec* iov, int iovcnt, size_t* count_out)
{
  ...
  // msg_iovlen is a size_t on Linux, an int on macOS, FreeBSD, DragonFly,
  // and Haiku and an unsigned int on OpenBSD. Let each platform convert
  // from int itself; a cast naming one of those types is wrong on the others.
