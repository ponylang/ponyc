...
static void final_small(chunk_t* chunk, uint32_t force_finalisers_mask)
{
  ...
  //    `force_finalisers_mask` should always be `0xFFFFFFFF` to make
  ...
  pony_assert(force_finalisers_mask == 0 || force_finalisers_mask == 0xFFFFFFFF);
...
void ponyint_heap_final(heap_t* heap)
{
  ...
  for(int i = 0; i < HEAP_SIZECLASSES; i++)
  {
    chunk_list(final_small, heap->small_free[i], 0xFFFFFFFF);
    chunk_list(final_small, heap->small_full[i], 0xFFFFFFFF);
  }
}
