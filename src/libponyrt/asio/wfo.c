...
static void wfo_signal_handler(int sig)
{
  if(sig >= MAX_SIGNAL)
    return;
...
static uint16 wfo_events_from_asio_flags(uint32_t flags)
{
  ...
  //          Since Haiku observes B_EVENT_INVALID, B_EVENT_ERROR and B_EVENT_DICONNECTED by default,
  //          we do not need to set them at all. So we set B_EVENT_INVALID only for ASIO_PROC,
  //          to allow us to handle it correclty without any other info available.
  if(flags & ASIO_PROC)
  {
  ...
  while((read_size = read_port_etc(b->port, &req, &msg, sizeof(msg), B_TIMEOUT, 0)) >= 0)
  {
    ...
    switch(req)
    {
      ...
      case WFO_SUBSCRIBE:
        ...
        type = (flags & ASIO_PROC) ? B_OBJECT_TYPE_THREAD : B_OBJECT_TYPE_FD;

        int32 result = -1;
        if(ev->wfo_id >= 0)
          result = wfo_event_update(b, ev, ev->wfo_id, ev->fd, type, events);
        else
          result = wfo_event_add(b, ev, ev->fd, type, events);
      ...
      case WFO_RESUBSCRIBE:
        ...
        if(wfo_event_update(b, ev, ev->wfo_id, ev->fd, type, events) < B_OK)
          pony_asio_event_send(ev, ASIO_ERROR, 0);
        break;
  ...
  while(true)
  {
    ...
    for(int32 i = 0; i < wait_count; i++)
    {
      ...
      if((events & (B_EVENT_DISCONNECTED | B_EVENT_INVALID | B_EVENT_ERROR)) > 0)
      {
        ...
        {
          // We use B_OBJECT_TYPE_THREAD for ASIO_PROC, or invalid FDs
          if(info->type != B_OBJECT_TYPE_THREAD)
          {
            flags |= ASIO_WRITE;
            ev->writeable = true;
          }
...
PONY_API void pony_asio_event_subscribe(asio_event_t* ev)
{
  ...
  if(ev->flags & (ASIO_READ | ASIO_WRITE | ASIO_PROC))
  {
...
PONY_API void pony_asio_event_unsubscribe(asio_event_t* ev)
{
  ...
  if(ev->flags & ASIO_SIGNAL)
  {
    ...
    // The event cleanup and sigaction restore are handled in
    // WFO_CANCEL_SIGNAL.
    // Don't send ASIO_ERROR on failure — the actor is tearing down anyway.
