...
typedef struct signal_subscribers_t {
  ...
  // The disposition in place before the first subscriber replaced it with
  // SIG_IGN, captured by sigaction's oldact at install and put back on
  // last-subscriber teardown. Restoring this instead of SIG_DFL returns a
  // signal the runtime had configured — SIGPIPE to SIG_IGN — to that
  // configuration, rather than leaving it to terminate the process on the
  // next delivery. Written by the first subscriber before it publishes
  // registered == 1 and read by the last canceler after it claims
        ...
        if(!has_subscribers)
        {
            ...
            &expected, -1, memory_order_seq_cst, memory_order_seq_cst))
          {
            ...
            if(still_empty)
            {
              ...
              // place before the first subscriber installed SIG_IGN (see
              // saved_action), remove the kevent registration. Teardown errors
              // are benign (matching unsubscribe's convention), so the
              // EV_DELETE is unchecked.
    ...
    while(true)
    {
      ...
      if(state == 0)
      {
          ...
          &expected, -1, memory_order_seq_cst, memory_order_seq_cst))
        {
          ...
          // oldact captures the disposition we are replacing, to restore on
          // the last unsubscribe (see saved_action).
          ...
          {
            // Undo the disposition change (put back what oldact captured
            // above) and reset the state before reporting, so a later
            // subscribe can retry cleanly.
            sigaction(sig, &subs->saved_action, NULL);
            atomic_store_explicit(&subs->registered, 0,
              memory_order_seq_cst);
            pony_asio_event_send(ev, ASIO_ERROR, SIGNAL_ERR_REFUSED);
