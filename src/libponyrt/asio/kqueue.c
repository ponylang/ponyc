...
typedef struct signal_subscribers_t {
  ...
  // signal the runtime had configured — SIGPIPE to SIG_IGN, the scheduler
  // sleep/wake signal to a no-op handler — to that configuration, rather than
  ...
  // canceler after it claims registered, so the registered protocol orders the
  // two with no extra sync.
