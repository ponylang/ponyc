...
PONY_API void pony_asio_event_resubscribe_read(asio_event_t* ev)
{
  ...
  {
    EV_SET(&event[i], ev->fd, EVFILT_READ, EV_ADD | kqueue_flags, 0, 0, (u_short)ev);
    i++;
  } else {
    return;
  }
...
PONY_API void pony_asio_event_resubscribe_write(asio_event_t* ev)
{
  ...
  {
    EV_SET(&event[i], ev->fd, EVFILT_WRITE, EV_ADD | kqueue_flags, 0, 0, (u_short)ev);
    i++;
  } else {
    return;
  }
...
PONY_API void pony_asio_event_subscribe(asio_event_t* ev)
{
  ...
  {
    EV_SET(&event[i], ev->fd, EVFILT_READ, EV_ADD | flags, 0, 0, (u_short)ev);
    i++;
  }

  if(ev->flags & ASIO_WRITE)
  {
    EV_SET(&event[i], ev->fd, EVFILT_WRITE, EV_ADD | flags, 0, 0, (u_short)ev);
    i++;
  }
  ...
  if(ev->flags & ASIO_TIMER)
  {
    ...
    EV_SET(&event[i], (uintptr_t)ev, EVFILT_TIMER, EV_ADD | EV_ONESHOT,
      0, ev->nsec / 1000000, (u_short)ev);
#else
    EV_SET(&event[i], (uintptr_t)ev, EVFILT_TIMER, EV_ADD | EV_ONESHOT,
      NOTE_NSECONDS, ev->nsec, (u_short)ev);
#endif
    i++;
  }
  ...
  if(ev->flags & ASIO_SIGNAL)
  {
    ...
    EV_SET(&event[i], ev->nsec, EVFILT_SIGNAL, EV_ADD | EV_CLEAR, 0, 0, (u_short)ev);
    i++;
  }
...
PONY_API void pony_asio_event_unsubscribe(asio_event_t* ev)
{
  ...
  if(ev->flags & ASIO_READ)
  {
    EV_SET(&event[i], ev->fd, EVFILT_READ, EV_DELETE, 0, 0, (u_short)ev);
    i++;
  }

  if(ev->flags & ASIO_WRITE)
  {
    EV_SET(&event[i], ev->fd, EVFILT_WRITE, EV_DELETE, 0, 0, (u_short)ev);
    i++;
  }

  if(ev->flags & ASIO_TIMER)
  {
    EV_SET(&event[i], (uintptr_t)ev, EVFILT_TIMER, EV_DELETE, 0, 0, (u_short)ev);
    i++;
  }
  ...
  if(ev->flags & ASIO_SIGNAL)
  {
    ...
    EV_SET(&event[i], ev->nsec, EVFILT_SIGNAL, EV_DELETE, 0, 0, (u_short)ev);
    i++;
  }
