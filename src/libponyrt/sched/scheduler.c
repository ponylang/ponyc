 ...
 * its noisy events, or termination was requested.  A termination
 * exit may carry a non-NULL actor that landed on the inject queue
 * between the TERMINATE decision and the pop; the caller runs it
 * harmlessly and terminates on the next quiescent check.
 */
static pony_actor_t* passive_wait(scheduler_t* sched)
{
  ...
  while(true)
  {
    ...
    // Scheduler 0 must keep triggering the cycle detector while
    // passive.  The steal loop's cycle-detector check sits after the
    // passive gate, so once scheduler 0 enters passive_wait it never
    ...
    // blocks flushed, foreign ones sent home, own dirty pages
    // decommitted, the oversized stash unmapped.
  ...
  while(true)
  {
    ...
    if(!ponyint_actor_getnoblock() && (sched->index == 0))
    {
      ...
      if(ponyint_cycle_check_blocked(last_cd_tsc, current_tsc))
      {
        ...
        if(actor != NULL)
        {
          // Run the cycle detector inline — without leaving steal and
          // unblocking — when quiescence is in progress or when ASIO is
          // no longer noisy.  The asio_stoppable gate is the final
          // round of the CNF/ACK protocol; the !asio_noisy gate covers
          // the earlier rounds where breaking out of steal to run the
          // CD would send UNBLOCK, resetting the protocol and
          // preventing quiescence from ever completing.
...
static void run_pinned_actors()
{
  ...
  while(true)
  {
    ...
    if(actor == NULL)
    {
      ...
      // pages decommitted, the oversized stash unmapped. Gating on the
      // backoff keeps a thread that briefly parks and resumes from
      // thrashing decommits; it rewarms its cache and pages when it runs.
      if(tick_ns >= SCHED_TICK_MAX_NS)
        ponyint_pool_return_idle();
