...
DEFINE_HASHMAP(reach_methods, reach_methods_t, reach_method_t,
  reach_method_hash, reach_method_cmp, NULL);
...
DEFINE_HASHMAP(reach_types, reach_types_t, reach_type_t,
  reach_type_hash, reach_type_cmp, reach_type_free);

DEFINE_HASHMAP(reach_type_cache, reach_type_cache_t, reach_type_t,
  reach_type_hash, reach_type_cmp, NULL);
...
static void reachable_ffi(reach_t* r, deferred_reification_t* reify,
  ast_t* ast, pass_opt_t* opt)
{
  ...
  bool reified;
  ast_t* return_typeargs = ast_childidx(ast, 1);
  if(ast_child(return_typeargs) == NULL)
  {
    // We don't have any specified return type at the call site,
    // use the type given by the declaration.
  ...
  ast_t* ast, pass_opt_t* opt)
{
  ...
  while(ast != NULL)
  {
    ...
    // Push all child expressions onto the stack for processing.
