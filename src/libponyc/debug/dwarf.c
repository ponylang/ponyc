  ...
  bool opaque, bool field)
{
  ...
  if(!opaque)
  {
    meta->size = pony_downcast(size_t, LLVMABISizeOfType(dwarf->target_data, type) << 3);
    meta->align = pony_downcast(size_t, LLVMABIAlignmentOfType(dwarf->target_data, type) << 3);
  }
}
  ...
  int index)
{
  ...
  meta.offset = pony_downcast(size_t, 8 * LLVMOffsetOfElement(dwarf->target_data, structure,
    offset + index));
