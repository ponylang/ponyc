...
static void start_function(compile_t *c, reach_type_t *t, reach_method_t *m,
                           LLVMTypeRef result, LLVMTypeRef *params,
                           unsigned count) {
  compile_method_t *c_m = (compile_method_t *)m->c_method;
...
static LLVMValueRef field_loc(compile_t *c, LLVMValueRef offset,
  ...
  LLVMValueRef size = LLVMConstInt(
      c->intptr, LLVMOffsetOfElement(c->target_data, structure, index), false);
...
static LLVMValueRef field_value(compile_t *c, LLVMTypeRef struct_type,
  ...
  LLVMValueRef field =
      LLVMBuildStructGEP2(c->builder, struct_type, object, index, "");
...
static void pointer_apply(compile_t *c, void *data, token_id cap) {
  ...
  LLVMValueRef loc =
      LLVMBuildInBoundsGEP2(c->builder, c_t_elem->mem_type, ptr, &index, 1, "");
...
static void pointer_offset(compile_t *c, void *data, token_id cap) {
  ...
  LLVMValueRef result =
      LLVMBuildInBoundsGEP2(c->builder, t_elem->mem_type, ptr, &n, 1, "");
...
static void pointer_delete(compile_t *c, reach_type_t *t,
  ...
  LLVMValueRef src =
      LLVMBuildInBoundsGEP2(c->builder, c_t_elem->mem_type, ptr, &n, 1, "");
  LLVMValueRef elen = LLVMBuildMul(c->builder, len, l_size, "");
...
static void nullable_pointer_create(compile_t *c, reach_type_t *t,
                                    compile_type_t *t_elem) {
  FIND_METHOD("create", TK_NONE);
...
static void nullable_pointer_is_none(compile_t *c, reach_type_t *t,
                                     token_id cap) {
...
static void donotoptimise_apply(compile_t *c, reach_type_t *t,
  ...
  LLVMTypeRef void_fn =
      LLVMFunctionType(c->void_type, &t_elem->use_type, 1, false);
  LLVMValueRef asmstr =
      LLVMConstInlineAsm(void_fn, "", "imr,~{memory}", true, false);
...
static void donotoptimise_observe(compile_t *c, reach_type_t *t, token_id cap) {
  ...
  LLVMValueRef asmstr =
      LLVMConstInlineAsm(void_fn, "", "~{memory}", true, false);
...
static void trace_array_elements(compile_t *c, reach_type_t *t,
                                 LLVMValueRef ctx, LLVMValueRef object,
  ...
  LLVMValueRef elem_ptr = LLVMBuildInBoundsGEP2(c->builder, c_t_elem->mem_type,
                                                pointer, &phi, 1, "");
  LLVMValueRef elem =
      LLVMBuildLoad2(c->builder, c_t_elem->mem_type, elem_ptr, "");
...
void genprim_array_serialise_trace(compile_t *c, reach_type_t *t) {
  ...
  c_t->serialise_trace_fn =
      codegen_addfun(c, genname_serialise_trace(t->name), c->trace_fn, true);
...
void genprim_array_serialise(compile_t *c, reach_type_t *t) {
  ...
  compile_type_t *c_t = (compile_type_t *)t->c_type;
  c_t->serialise_fn =
      codegen_addfun(c, genname_serialise(t->name), c->serialise_fn, true);
  ...
  LLVMValueRef offset_addr =
      LLVMBuildInBoundsGEP2(c->builder, c->i8, addr, &offset, 1, "");
  ...
  LLVMValueRef test =
      LLVMBuildICmp(c->builder, LLVMIntNE, mut,
  ...
  args[0] = ctx;
  args[1] = ptr;
  LLVMValueRef ptr_offset =
      gencall_runtime(c, "pony_serialise_offset", args, 2, "");

  LLVMValueRef ptr_loc = field_loc(c, offset_addr, c_t->structure, 3);
  LLVMBuildStore(c->builder, ptr_offset, ptr_loc);

  LLVMValueRef ptr_offset_addr =
      LLVMBuildInBoundsGEP2(c->builder, c->i8, addr, &ptr_offset, 1, "");
  ...
  if ((t_elem->underlying == TK_PRIMITIVE) && (c_t_e->primitive != NULL)) {
    ...
    LLVMValueRef elem_ptr =
        LLVMBuildInBoundsGEP2(c->builder, c_t_e->mem_type, ptr, &phi, 1, "");
...
void genprim_array_deserialise(compile_t *c, reach_type_t *t) {
  ...
  compile_type_t *c_t = (compile_type_t *)t->c_type;
  c_t->deserialise_fn =
      codegen_addfun(c, genname_deserialise(t->name), c->trace_fn, true);
  ...
  LLVMValueRef ptr_loc =
      LLVMBuildStructGEP2(c->builder, c_t->structure, object, 3, "");
...
void genprim_string_serialise_trace(compile_t *c, reach_type_t *t) {
  ...
  LLVMValueRef alloc =
      LLVMBuildAdd(c->builder, size, LLVMConstInt(c->intptr, 1, false), "");
...
void genprim_string_deserialise(compile_t *c, reach_type_t *t) {
  ...
  LLVMValueRef ptr_addr =
      gencall_runtime(c, "pony_deserialise_block", args, 3, "");
...
static void platform_runtimestatsmessages(compile_t *c, reach_type_t *t,
                                          token_id cap) {
  ...
  LLVMValueRef result =
      LLVMConstInt(c->i1, runtimestatsmessages_enabled, false);
  genfun_build_ret(c, result);
  codegen_finishfun(c);
}
...
static LLVMBasicBlockRef handle_nan(compile_t *c, LLVMValueRef arg,
                                    LLVMTypeRef int_type, uint64_t exp,
                                    uint64_t mantissa) {
...
static LLVMValueRef handle_overflow_saturate(compile_t *c, LLVMValueRef arg,
                                             LLVMTypeRef from, LLVMTypeRef to,
                                             LLVMValueRef to_max,
                                             LLVMValueRef to_min, bool sign) {
  ...
  LLVMValueRef is_overflow =
      LLVMBuildFCmp(c->builder, LLVMRealOGT, arg, to_fmax, "");
  ...
  LLVMValueRef is_underflow =
      LLVMBuildFCmp(c->builder, LLVMRealOLT, arg, to_fmin, "");
...
static LLVMValueRef f32_to_si_saturation(compile_t *c, LLVMValueRef arg,
                                         num_conv_t *to) {
  LLVMBasicBlockRef test_overflow =
      handle_nan(c, arg, c->i32, 0x7F800000, 0x007FFFFF);
  ...
  to_max =
      LLVMBuildLShr(c->builder, to_min, LLVMConstInt(to->type, 1, false), "");
...
static LLVMValueRef f64_to_si_saturation(compile_t *c, LLVMValueRef arg,
                                         num_conv_t *to) {
  LLVMBasicBlockRef test_overflow =
      handle_nan(c, arg, c->i64, 0x7FF0000000000000, 0x000FFFFFFFFFFFFF);
  genfun_build_ret(c, LLVMConstNull(to->type));
  LLVMPositionBuilderAtEnd(c->builder, test_overflow);
  LLVMValueRef to_max = LLVMConstNull(to->type);
  LLVMValueRef to_min = LLVMBuildNot(c->builder, to_max, "");
...
static LLVMValueRef f32_to_u128_saturation(compile_t *c, LLVMValueRef arg) {
  ...
  LLVMValueRef is_overflow =
      LLVMBuildFCmp(c->builder, LLVMRealOGE, arg, max, "");
  LLVMBuildCondBr(c->builder, is_overflow, overflow, test_underflow);
  ...
  LLVMPositionBuilderAtEnd(c->builder, test_underflow);
  LLVMValueRef is_underflow =
      LLVMBuildFCmp(c->builder, LLVMRealOLT, arg, min, "");
  LLVMBuildCondBr(c->builder, is_underflow, underflow, normal);
...
static LLVMValueRef f64_to_f32_saturation(compile_t *c, LLVMValueRef arg) {
  ...
  LLVMValueRef is_overflow =
      LLVMBuildFCmp(c->builder, LLVMRealOGT, arg, f32_max, "");
  LLVMBuildCondBr(c->builder, is_overflow, overflow, test_underflow);
  ...
  LLVMValueRef is_underflow =
      LLVMBuildFCmp(c->builder, LLVMRealOLT, arg, f32_min, "");
  LLVMBuildCondBr(c->builder, is_underflow, underflow, normal);
...
static LLVMValueRef u128_to_f32_saturation(compile_t *c, LLVMValueRef arg) {
  ...
  f32_max = LLVMBuildFPExt(c->builder, f32_max, c->f64, "");
  LLVMValueRef is_overflow =
      LLVMBuildFCmp(c->builder, LLVMRealOGT, val_f64, f32_max, "");
...
static void number_conversion(compile_t *c, void **data, token_id cap) {
  ...
  if (!native128 && ((from->is_float && (to->size > 64)) ||
...
static void number_conversions(compile_t *c) {
  num_conv_t ilp32_conv[] = {{"I8", "i8", c->i8, 8, true, false},
  ...
  num_conv_t lp64_conv[] = {{"I8", "i8", c->i8, 8, true, false},
                            {"I16", "i16", c->i16, 16, true, false},
                            {"I32", "i32", c->i32, 32, true, false},
                            {"I64", "i64", c->i64, 64, true, false},
  ...
  num_conv_t llp64_conv[] = {{"I8", "i8", c->i8, 8, true, false},
                             {"I16", "i16", c->i16, 16, true, false},
                             {"I32", "i32", c->i32, 32, true, false},
                             {"I64", "i64", c->i64, 64, true, false},
...
static void f32__inf(compile_t *c, reach_type_t *t) {
  ...
  LLVMValueRef result =
      LLVMBuildSelect(c->builder, sign, LLVMConstInf(c->f32, true),
                      LLVMConstInf(c->f32, false), "");
  genfun_build_ret(c, result);
  codegen_finishfun(c);
}
...
static void f32_from_bits(compile_t *c, reach_type_t *t) {
  ...
  LLVMValueRef result =
      LLVMBuildBitCast(c->builder, LLVMGetParam(c_m->func, 1), c->f32, "");
  genfun_build_ret(c, result);
  codegen_finishfun(c);
}
...
static void f32_bits(compile_t *c, reach_type_t *t, token_id cap) {
  ...
  LLVMValueRef result =
      LLVMBuildBitCast(c->builder, LLVMGetParam(c_m->func, 0), c->i32, "");
  genfun_build_ret(c, result);
  codegen_finishfun(c);
}
...
static void f64__inf(compile_t *c, reach_type_t *t) {
  ...
  LLVMValueRef sign = LLVMGetParam(c_m->func, 1);
  LLVMValueRef result =
      LLVMBuildSelect(c->builder, sign, LLVMConstInf(c->f64, true),
                      LLVMConstInf(c->f64, false), "");
  genfun_build_ret(c, result);
  codegen_finishfun(c);
}
...
static void f64_from_bits(compile_t *c, reach_type_t *t) {
  ...
  LLVMValueRef result =
      LLVMBuildBitCast(c->builder, LLVMGetParam(c_m->func, 1), c->f64, "");
  genfun_build_ret(c, result);
  codegen_finishfun(c);
}
...
static void f64_bits(compile_t *c, reach_type_t *t, token_id cap) {
  ...
  LLVMValueRef result =
      LLVMBuildBitCast(c->builder, LLVMGetParam(c_m->func, 0), c->i64, "");
  genfun_build_ret(c, result);
  codegen_finishfun(c);
}
...
static void make_cpuid(compile_t *c) {
  if (target_is_x86(c->opt->triple)) {
    ...
    LLVMValueRef cpuid =
        LLVMGetInlineAsm(f_type, "cpuid", 5, "={ax},={bx},={cx},={dx},{ax}", 28,
                         false, false, LLVMInlineAsmDialectATT, false);
    LLVMValueRef arg = LLVMGetParam(fun, 0);

    LLVMValueRef result =
        LLVMBuildCall2(c->builder, f_type, cpuid, &arg, 1, "");
    genfun_build_ret(c, result);
...
static void make_rdtscp(compile_t *c) {
  if (target_is_x86(c->opt->triple)) {
    ...
    LLVMTypeRef r_type =
        LLVMStructTypeInContext(c->context, r_type_fields, 2, false);
    LLVMTypeRef f_type_r = LLVMFunctionType(r_type, NULL, 0, false);
    LLVMValueRef rdtscp =
        LLVMAddFunction(c->module, "llvm.x86.rdtscp", f_type_r);

    // i64 @internal.x86.rdtscp(i32*)
    LLVMTypeRef ptr_type = c->ptr;
    LLVMTypeRef f_type_f = LLVMFunctionType(c->i64, &ptr_type, 1, false);
    LLVMValueRef fun =
        codegen_addfun(c, "internal.x86.rdtscp", f_type_f, false);
    LLVMSetFunctionCallConv(fun, LLVMCCallConv);

    codegen_startfun(c, fun, NULL, NULL, NULL, false);
    LLVMValueRef result =
        LLVMBuildCall2(c->builder, f_type_r, rdtscp, NULL, 0, "");
...
static void make_cntvct_el0(compile_t *c) {
  if (target_is_arm(c->opt->triple)) {
    // i64 @internal.aarch64
    LLVMTypeRef f_type_f = LLVMFunctionType(c->i64, &c->void_type, 0, false);
    LLVMValueRef fun =
        codegen_addfun(c, "internal.aarch64.cntvct_el0", f_type_f, false);

    LLVMSetFunctionCallConv(fun, LLVMCCallConv);
    codegen_startfun(c, fun, NULL, NULL, NULL, false);
    LLVMValueRef get_cntvct_el0 =
        LLVMGetInlineAsm(f_type_f, "MRS $0, CNTVCT_EL0", 18, "=r", 2, false,
...
static void make_pmccntr_el0(compile_t *c) {
  if (target_is_arm(c->opt->triple)) {
    ...
    codegen_startfun(c, fun, NULL, NULL, NULL, false);
    LLVMValueRef get_cntvct_el0 =
        LLVMGetInlineAsm(f_type_f, "MRS $0, PMCCNTR_EL0", 19, "=r", 2, false,
                         false, LLVMInlineAsmDialectATT, false);
