...
static void set_runtime_return_attrs(compile_t* c)
{
  ...
  // The alignment values must match what the runtime actually provides:
  // HEAP_MIN for heap allocations, POOL_MIN for pool allocations.
  // For heap allocations this is load-bearing for HeapToStack, which
  // derives the promoted alloca's alignment from this attribute.
...
static bool init_module(compile_t* c, ast_t* program, pass_opt_t* opt)
{
  ...
  // All symbols get external linkage so that LTO's internalization pass
  // can determine final visibility.
...
bool codegen_merge_runtime_bitcode(compile_t* c)
{
  ...
  if(runtime == NULL)
  {
    c->runtime_bitcode_merged = false;
    return true;
  }
  ...
  ast_t* package)
{
  ...
  snprintf(module_name, sizeof(module_name), "%s.%s",
    c->filename, (pkg_sym != NULL) ? pkg_sym : package_filename(package));
...
void codegen_init_modules(compile_t* c, ast_t* program)
{
  ...
  while(package != NULL)
  {
    if(ast_id(package) == TK_PACKAGE)
    {
      ...
      for(size_t i = 0; i < c->per_module_count; i++)
      {
        if(c->per_module_states[i].package == package)
        {
          found = true;
          break;
        }
...
size_t codegen_module_index_for_package(compile_t* c, ast_t* package)
{
  for(size_t i = 0; i < c->per_module_count; i++)
  {
    if(c->per_module_states[i].package == package)
...
static void copy_function_attributes(LLVMValueRef src, LLVMValueRef dst)
{
  unsigned param_count = LLVMCountParams(src);

  for(unsigned idx = 0; idx <= param_count + 1; idx++)
  {
    LLVMAttributeIndex ai;

    if(idx == 0)
      ai = LLVMAttributeFunctionIndex;
    else if(idx == 1)
      ai = LLVMAttributeReturnIndex;
    else
      ai = idx - 1;

    unsigned count = LLVMGetAttributeCountAtIndex(src, ai);

    if(count == 0)
      continue;

    LLVMAttributeRef stack_attrs[16];
    LLVMAttributeRef* attrs = (count <= 16) ? stack_attrs :
      (LLVMAttributeRef*)malloc(count * sizeof(LLVMAttributeRef));
    LLVMGetAttributesAtIndex(src, ai, attrs);

    for(unsigned i = 0; i < count; i++)
      LLVMAddAttributeAtIndex(dst, ai, attrs[i]);
...
LLVMValueRef codegen_resolve_function(compile_t* c, LLVMValueRef fn)
{
  ...
  copy_function_attributes(fn, resolved);
  return resolved;
}
