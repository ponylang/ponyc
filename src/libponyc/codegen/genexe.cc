  ...
  const char* sys_triple, uint16_t target_machine, strtable_t* strtab)
{
  ...
  {
    snprintf(buf, sizeof(buf), "%s%s%s/crti.o", use_sysroot ? sysroot : "", use_sysroot ? "/" : "", candidates[i]);
    if(file_exists(buf) && elf_matches_target(buf, target_machine))
    {
      snprintf(buf, sizeof(buf), "%s%s%s", use_sysroot ? sysroot : "", use_sysroot ? "/" : "", candidates[i]);
      free(candidates);
