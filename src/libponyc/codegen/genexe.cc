...
// The use of NDEBUG instead of PONY_NDEBUG here is intentional.
  ...
  for(size_t i = 0; i < path_count; i++)
  {
    ...
    // On Haiku installations go to one of standard paths,
    // so there's no need to support non-standard installations.
    // This also would end up with incorrect RUNPATHs for pony-[lsp|doc|lint]
    // because `cmake --install` does not correct RUNPATHS, and they
    // keep paths for build environment instead of runtime environment.
