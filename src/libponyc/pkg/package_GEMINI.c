...
ast_t* package_load(ast_t* from, const char* path, pass_opt_t* opt)
{
  ...
  if(package != NULL)
  {
    if(ast_checkflag(package, AST_FLAG_PRESERVE))
      return NULL;

    if(ast_checkflag(package, AST_FLAG_RECURSE_1))
      return package;

    ast_setflag(package, AST_FLAG_RECURSE_1);
    if(!ast_passes_subtree(&package, opt, opt->program_pass))
    {
      ast_clearflag(package, AST_FLAG_RECURSE_1);
