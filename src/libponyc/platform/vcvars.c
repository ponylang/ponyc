...
static bool find_kernel32(vcvars_t* vcvars, errors_t* errors)
{
  ...
  if(strcmp("v10.0", sdk.name) == 0)
  {
    ...
    // dirs the C-shim compile (genc) needs to resolve <stdio.h>, <windows.h>.
    {
...
// genc supplies a default version in that case.
  ...
  if (find_executable(install_path, link_path, vcvars->link, true, errors))
  {
    ...
    if (find_executable(install_path, lib_path, vcvars->ar, false, errors))
    {
      ...
      strncat(vcvars->msvc_include, info->include_path,
        MAX_PATH - strlen(vcvars->msvc_include));
...
bool vcvars_get(pass_opt_t* opt, vcvars_t* vcvars, errors_t* errors)
{
  ...
  // (genexe) for the lib dirs and the C-shim compiler (genc) for the include
  ...
  // memoization find_macos_sdk_include uses in genc.cc). Only successes are
