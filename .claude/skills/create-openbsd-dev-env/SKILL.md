...
- **Raise the `datasize` limit.** OpenBSD caps a process's data segment in
  ...
  raises `datasize-max`/`-cur` to at least 8192M. Without it the libs build dies with
...
Linux. `-smp`/`-m` are speed knobs; CI uses 4 CPUs / 12G. The build is capped by the 8192M
...
must end up at least `8192M` (what CI sets). On the stock OpenBSD 7.9 image both default to
`1536M`, so the two `sed`s below do it. But the goal is the `8192M` ceiling, not the literal
`1536M`: if a future image ships a different default the `sed`s match nothing, so read the
current values (the trailing `grep` prints them) and, if either is below `8192M`, raise it
to `8192M` directly — adjust the `sed` pattern or edit `/etc/login.conf`. Confirm both read
`8192M` before continuing; too low and the libs build dies with allocation failures partway
...
# Raise the openbsd user's datasize cap to 8192M. The stock 7.9 image
# defaults both to 1536M; if a future image differs, the sed matches
# nothing — read the grep below and set both to 8192M however fits.
doas sed -i 's/datasize-max=1536M/datasize-max=8192M/' /etc/login.conf
doas sed -i 's/datasize-cur=1536M/datasize-cur=8192M/' /etc/login.conf
grep -E 'datasize-(cur|max)=' /etc/login.conf   # confirm both now read 8192M
EOF
```
...
each parallel compile is bounded by the 8192M `datasize` cap, so higher parallelism is the
...
- `-smp 8` / `-m 12G` for build speed (CI uses 4 / 12G), and a `-pidfile` for the lifecycle
  commands.
