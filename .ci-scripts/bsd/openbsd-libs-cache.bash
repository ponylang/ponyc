#!/usr/bin/env bash
#
# Run the in-VM libs-cache handling for the OpenBSD tier-3 job over ssh: either
...
# already booted by openbsd-provision.bash, reachable at
# 'ssh -i vm_key -p 2222 openbsd@localhost' with the checkout under /build/ponyc.
# Run from the checkout root (where the provision step left vm_key).
#
# Usage: openbsd-libs-cache.bash <operation>
...
operation=${1:?usage: openbsd-libs-cache.bash <operation>}
...
# OpenBSD builds on a dedicated, smaller /build disk (its default disklabel
# confines root), so the build ops free the intermediate LLVM tree once the libs
...
ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -i vm_key -p 2222 openbsd@localhost /bin/sh <<REMOTE
set -e
cd /build/ponyc
