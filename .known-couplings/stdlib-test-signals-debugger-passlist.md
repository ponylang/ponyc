# Signals raised by stdlib tests must be in the Makefile debugger pass-lists

The stdlib test suites run under `$(debuggercmd)` on the glibc Linux CI legs
(per-PR and weekly): an lldb or gdb wrapper whose pass-lists name the signals
the debugger must forward without stopping (`process handle <SIG> --pass true
