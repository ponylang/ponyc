actor Buffer
  ...
  be permission_to_consume(cons: Consumer) =>
    ...
    _env.out.print(debug_string)
    try
      _env.out.print(debug_string + ": Calling consumer to consume")
      cons.consuming(_products.delete(0)) // Fails if products is empty.
      try
        _env.out.print(debug_string + ": Calling producer to produce")
        _producers_waiting.delete(0).produce()
      end  // If there are no producers in waiting, do nothing.
    else
      _env.out.print(debug_string + ": Storing consumer in waiting")
  ...
  be permission_to_produce(prod: Producer) =>
    ...
    if _products.size() < capacity then
      ...
      _env.out.print(debug_string + ": Storing producer in waiting")
    end
  ...
  be store_product(product: Product) =>
    ...
    try
      ...
      _env.out.print(debug_string + ": Storing product")
