...
- `use "cincludedir:PATH"` adds an include search directory (clang's `-I`). Relative paths resolve against the package's directory.
...
Once `cshim` has been compiled, in the same directory as this README file run `./cshim`. You should see the macro value from `cdefine:` and the constant from the header found via `cincludedir:`:
