...
class val LspPositionRange
  """
  LSP range with inclusive start and exclusive end.
  """
...
class val LspPosition
  ...
  new val from_ast_pos_end(position: Position) =>
    """
    Convert AST position to LSP position for range
    end (exclusive). AST positions are inclusive
    (point to the last character), but LSP range ends
    are exclusive (point one past the last character).
    """
    _line = (position.line() - 1).max(0)
