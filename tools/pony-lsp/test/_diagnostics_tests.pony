...
class \nodoc\ iso _DiagnosticTest is UnitTest
  fun name(): String => "diagnostics/compiler_error_to_diagnostics"
  ...
  fun apply(h: TestHelper) =>
    ...
    let harness = TestHarness.create(
      ...
      object iso is MessageHandler
        ...
        fun ref handle_notification(h: TestHelper, notification: Notification, server: BaseProtocol) =>
          ...
          | Methods.text_document().publish_diagnostics() =>
            try
              for diagnostic in JsonNav(notification.params)("diagnostics").as_array()?.values() do
                received_diagnostics = received_diagnostics + 1
                h.log("received diagnostic " + received_diagnostics.string() + ": " + notification.json().string())
                // strip off linebreak from error message
                let message: String val = recover val JsonNav(diagnostic)("message").as_string()?.clone().>strip() end
                h.assert_true(expected_messages.contains(message, {(l, r) => l == r}), "Unexpected diagnostic message: '" + message + "'")
                if received_diagnostics == 5 then
                  h.complete(true)
                end
              end
            else
              h.fail("Weird diagnostics notification: " + notification.string())
            end
          end

        fun ref handle_response(h: TestHelper, res: ResponseMessage, server: BaseProtocol) =>
      ...
      where after_sends = 3, after_logs = USize.max_value()
    )
