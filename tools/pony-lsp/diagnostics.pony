...
class val Diagnostic
  ...
  new val from_error(err: Error) =>
    """
    Create a Diagnostic from a compiler Error.
    """
    this.location =
      LspLocation(
        Uris.from_path(
        ...
        LspPositionRange.from_single_pos(
          LspPosition.from_ast_pos(err.position))
      )
  ...
  fun val expand_related(): Array[Diagnostic] val =>
    ...
    they otherwise wouldn't show related information.
    """
    let expanded =
      recover trn
        Array[Diagnostic]
...
class val DiagnosticRelatedInformation
  """
  Related information for a diagnostic, pointing to
