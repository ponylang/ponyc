...
class PackageState
  """
  State of a compiled package, containing its modules
  and document states.
  """
  let path: FilePath
  ...
  fun debug(): String val =>
    "Package " + this.path.path + " (" + (
      ...
      Iter[(String box, DocumentState box)](
        documents.pairs())
        .map[String]({(kv) =>
          kv._1 + " (" +
  ...
  fun ref insert_new(
    document_path: String): DocumentState
  =>
    """
    Insert a new module by the given `document_path`
    into this package.
    """
    let doc_state =
      DocumentState.create(
        document_path, this._channel)

    try
      // populate the document from the last
      // compilation result if available
      let pkg = this.package() as Package
      let module =
        pkg.find_module(document_path) as Module
  ...
  fun ref ensure_document(
    document_path: String): DocumentState
  =>
    """
    Returns the document state in a tuple together
    with a boolean, denoting whether this documents
    needs compilation.
    """
    match \exhaustive\ this.get_document(
      document_path)
    | let d: DocumentState =>
      d
    | None =>
  ...
  fun ref update(
    result: Package val, run_id: USize)
  =>
    """
    ...
    if run_id >= this._compiler_run_id then
      ...
      this._channel.log(
        "Updating package " + result.path)
      this._channel.log(this.debug())
      // for each open document, update the
      // document state if we have a module for it
      for (doc_path, doc_state) in
        this.documents.pairs()
      do
        // TODO: ensure both module and
        // package-state paths are normalized
        match \exhaustive\ result
          .find_module(doc_path)
        | let m: Module val =>
          // update each document state
          doc_state.update(run_id, m)
        | None =>
          this._channel.log(
            "No module found for " + doc_path)
        end
      end
    end

  fun ref add_diagnostic(
    run_id: USize, diagnostic: Diagnostic)
  =>
    if run_id >= this._compiler_run_id then
      this._compiler_run_id = run_id
      // TODO:
...
class DocumentState
  ...
  new create(path': String, channel': Channel) =>
    ...
    _document_symbols =
      _document_symbols.empty()
    _hash = _hash.empty()
    compiler_run_id = 0

  fun ref update(
    run_id: USize, module': Module val)
  =>
  ...
  fun ref add_diagnostic(
    run_id: USize, diagnostic: Diagnostic)
  =>
    // also accept diagnostics from the
    // last/current run, as they trickle in one
    // after the other
    if run_id == this.compiler_run_id then
  ...
  fun box needs_compilation(): Bool =>
    (this.module() is None) and
      (this.diagnostics() is None)

  fun box module(): (Module val | None) =>
    this._module.get(compiler_run_id)

  fun box diagnostics():
    (Array[Diagnostic] box | None)
  =>
    this._diagnostics.get(compiler_run_id)

  fun box position_index():
    (PositionIndex val | None)
  =>
  ...
  =>
    ...
    | let mod: Module val =>
      let created =
        DocumentSymbols.from_module(
          mod, this._channel)
      this._document_symbols.update(
        this.compiler_run_id, created)
      created
    else
      // no module available yet, no
      // documentsymbols available yet
      this._channel.log(
        "No module available yet for " +
        this.path)
      []
    end
...
class ref FromCompilerRun[T: Any #read]
  """
  Something associated with a certain compiler run,
  identified by a `USize`.
  """
  var _thing: (T | None)
  var compiler_run_id: USize
  ...
  fun get(
    current_run_id: USize): (this->T | None)
  =>
