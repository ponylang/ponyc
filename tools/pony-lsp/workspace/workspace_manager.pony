...
actor WorkspaceManager
  ...
  A workspace is initially created by scanning its
  directory and extracting information from available
  `corral.json` files.

  More information is obtained during execution of the
  type-checking pipeline of the libponyc compiler.
  After first successful compilation we should have all
  information at hand to do LSP work.
  """
  ...
  var _current_request:
    (RequestMessage val | None) = None
  var _compiling: Bool = false
  var _awaiting_compilation_for:
    Map[String, (USize | None)] =
  ...
  fun ref _ensure_package(
    package_path: FilePath): PackageState
  =>
    try
      ...
      this._packages.insert(
        package_path.path,
        PackageState.create(
          package_path, this._channel))
    end

  fun _get_package(
    package_path: FilePath
  ): (this->PackageState | None) =>
    try
      this._packages(package_path.path)?
    else
      None
  ...
  fun box _find_workspace_package(
    document_path: String): FilePath ?
  =>
    """
  ...
  be done_compiling(
    ...
    result:
      (Program val | Array[Error val] val),
    run: USize)
  =>
    """
    ...
    """
    if this._client
      .supports_window_work_done_progress()
    then
      let message =
    ...
    if run > this._compile_run then
      ...
      let errors_by_file =
        Map[String, pc.Vec[JsonValue]].create(4)
      ...
      | let program: Program val =>
        ...
        for package in program.packages() do
          let package_path =
            FilePath(this._file_auth, package.path)
          let package_state =
            this._ensure_package(package_path)
          ...
          do
            let await_pkg_path =
              Path.base(await_comp_file)
            if package_state.path.path ==
              await_pkg_path
            then
              try
                ...
                let new_mod_hash =
                  (package_state.get_document(
                    await_comp_file)
                      as DocumentState)
                    .module_hash()
                this._awaiting_compilation_for
                  .remove(await_comp_file)?
          ...
          | let err_file_uri: String val =>
            ...
            try
              let package: FilePath =
                this._find_workspace_package(
                  err_file)?
              let package_state =
                this._ensure_package(package)
              let document_state =
                package_state
                  .ensure_document(err_file)
              document_state.add_diagnostic(
                run, diagnostic)
            end
            ...
            then
              ...
              errors_by_file.upsert(
                err_file,
                pc.Vec[JsonValue]
                  .push(diagnostic.to_json()),
                {(current: pc.Vec[JsonValue],
                  provided: pc.Vec[JsonValue]) =>
      ...
      this._channel.log(
        "Received compilation result for run " +
        run.string() +
        " but already received results for run " +
  ...
  fun ref _compile(package: FilePath) =>
    ...
    """
    this._channel.log(
      "Compiling package " + package.path +
      " with dependency-paths: " +
      ", ".join(
        workspace.dependency_paths.values()))
  ...
  fun _compilation_token(
    package: FilePath): String
  =>
    recover val
      String.create(128)
        .> append("pony-lsp")
  ...
  =>
    ...
    this._channel.log(
      "handling did_open of " + document_path)
    ...
    let package: FilePath =
      try
        ...
        if not Path.is_abs(package_path) then
          ...
          FilePath(
            this._file_auth, package_path)
        end
      end
    this._channel.log(
      "did_open in pony package @ " +
      package.path)
    let package_state =
      this._ensure_package(package)
    let doc_state =
      package_state.ensure_document(
        document_path)
    if doc_state.needs_compilation() then
      if this._compiling then
        let current_hash =
          doc_state.module_hash()
        this._awaiting_compilation_for.insert(
          document_path, current_hash)
      else
        _channel.log(
          "No module found for document " +
          document_path +
          ". Need to compile.")
  ...
  =>
    ...
    let document_path =
      Uris.to_path(document_uri)
    this._channel.log(
      "handling did_close of " + document_path)
    try
      let package: FilePath =
        this._find_workspace_package(
          document_path)?
      ...
      try
        let document_state =
          package_state.documents
            .remove(document_path)?._2
  ...
  =>
    ...
    let document_path =
      Uris.to_path(document_uri)
    this._channel.log(
      "handling did_save of " + document_path)
    try
      let package: FilePath =
        this._find_workspace_package(
          document_path)?
      ...
      if this._compiling then
        ...
        this._awaiting_compilation_for.insert(
          document_path, text_content_hash)
      else
        this._compile(package)
      end
    else
  ...
  =>
    ...
    (let line, let column) =
      match \exhaustive\ _parse_hover_position(
        request)
    ...
    | let hover_node: AST box =>
      ...
      // response
      match \exhaustive\ HoverFormatter
        .create_hover(hover_node, this._channel)
      | let hover_text: String =>
        // Use the original hover node for
        // highlighting, not any definition it
        // may reference
        let hover_response =
    ...
    | None =>
      this._channel.log(
        "No hover data found for position")
    end
  ...
  fun ref _parse_hover_position(
    ...
    """
    Extract line and column from hover request
    params. Sends error response and returns None
    if invalid.
    """
    try
      let nav =
  ...
  fun _build_hover_response(
    hover_text: String,
    ast: AST box): JsonValue
  =>
    """
    Build the LSP Hover response JSON from hover
    text and AST node.
    """
  ...
  =>
    ...
    """
    this._channel.log(
      "Handling textDocument/documentSymbol")
    let document_path =
      Uris.to_path(document_uri)
    try
      let package: FilePath =
  ...
  =>
    ...
    """
    this._channel.log(
      "Handling textDocument/diagnostic")
    let document_path =
      Uris.to_path(document_uri)
    ...
    try
      ...
      this._channel.log(
        "[textDocument/diagnostic] " +
        "Unable to find workspace package " +
        "for: " + document_uri)
    end
    this._channel.send(
      ResponseMessage.create(
  ...
  fun _get_node_for_highlight(
    ast: AST box): AST box
  =>
    """
    Get the appropriate node for hover highlighting.
    For declarations (class, fun, let, etc.), return
    the identifier child. For references and type
    nodes, find the identifier within them. Otherwise
