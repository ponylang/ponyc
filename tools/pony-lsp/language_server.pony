...
actor LanguageServer is (Notifier & RequestSender)
  ...
  var _expect_responses_for:
    Array[(RequestId, String, (ResponseNotify | None))]
  ...
  =>
    ...
    _expect_responses_for =
      _expect_responses_for.create(4)
  ...
  fun tag _get_document_uri(
    ...
    JsonNav(params)("textDocument")("uri")
      .as_string()?

  be handle_parse_error(err: ParseError val) =>
    this._channel.log(
      "Parse Error: " + err.string(), Warning)
  ...
  fun ref handle_request(r: RequestMessage val) =>
    ...
    | _Initialized =>
          ...
          this._channel.send(
            ResponseMessage.create(
              ...
              ResponseError(
                ErrorCodes.internal_error(),
                "[" + r.method + "] " +
                "No workspace found for request '" +
                r.json().string() + "'")
            )
          )
        end
      ...
      | Methods.shutdown() =>
        ...
        this._channel.send(
          ResponseMessage.create(r.id, None))
      else
        this._channel.send(
          ResponseMessage.create(
            r.id,
  ...
  fun ref handle_response(r: ResponseMessage val) =>
    ...
    | _Initialized | _Uninitialized =>
      for i in
        Range[USize](
          0, this._expect_responses_for.size())
      do
        try
          ...
          then
            ...
            | Methods.client().register_capability() =>
              this
                .handle_register_capability_response(r)
            | Methods.window()
              .work_done_progress().create()
            =>
              this
                .handle_work_done_progress_create_response(r)
            | let other: String =>
              this._channel.log(
                "Unhandled response method " + other)
            end
            match maybe_notify
            | let rn: ResponseNotify =>
              rn.notify(expected_method, r)
            end
  ...
  fun ref handle_notification(n: Notification val) =>
    ...
    | Methods.text_document().did_open() =>
      try
        let document_uri =
          _get_document_uri(n.params)?
        // TODO: extract params into class
        // according to spec
        (_router.find_workspace(document_uri)
          as WorkspaceManager)
    ...
    =>
      try
        let settings =
          JsonPathParser
            .compile("$.settings")?
            .query_one(n.params) as JsonValue
        this.handle_did_change_configuration(
          settings)
      else
        this._channel.log(
          "[" + n.method +
      ...
      this._channel.log(
        "Method not implemented: " + n.method)
    end
  ...
  =>
    ...
    | let params: JsonObject val =>
      ...
      this._client = client
      this._channel.log(
        "Connected to client: " + client.string())
      // extract workspace folders, rootUri,
      // rootPath in that order:
      ...
      let scanner =
        WorkspaceScanner.create(this._channel)
      ...
      | let workspace_str: String =>
        try
          ...
          let pony_workspaces =
            scanner.scan(
              this._file_auth, workspace_str)
          for pony_workspace in
            pony_workspaces.values()
          do
            let mgr =
            ...
            this._channel.log(
              "Adding workspace " +
              pony_workspace.debug())
            this._router.add_workspace(
              pony_workspace.folder, mgr)?
          end
        end
      | let workspace_arr: JsonArray =>
        for workspace_obj in
          workspace_arr.values()
        do
          try
  ...
  fun ref _next_request_id(): RequestId =>
    this._request_id_gen =
      this._request_id_gen + 1

  fun ref handle_initialized(
    notification: Notification val)
  =>
    """
    Handle the initialized notification from the
    client.
    """
    // register for did change configuration,
    // if supported
    var can_get_settings = false
    try
      if (this._client as Client)
        .supports_configuration_dynamic_registration()
      then
    ...
    // if none of these work, we have no way to
    // get our config :(
  ...
  fun ref handle_did_change_configuration(
    settings: JsonValue val)
  =>
    // example output: for settings:
    // {"pony-lsp":{"defines":["FOO","BAR"]}}
    this._channel.log(
      "Received didChangeConfiguration " +
      "response: " + settings.string())
    match settings
    | None =>
      // this is a signal to pull the settings
      // again, so we do it here
      try
        this._channel.log(
          "Received null " +
          "didChangeConfiguration response: " +
    ...
    | let json_settings: JsonObject =>
      let maybe_settings =
        try
          let json_settings_obj =
            JsonNav(json_settings)("pony-lsp")
              .as_object()?
      ...
      this._channel.log(
        ...
        "No or invalid pony-lsp settings " +
        "provided.")
  ...
  fun handle_configuration(
    r: ResponseMessage val)
  =>
    let maybe_settings =
      try
        // LSPAny[] - we only read the first one,
        // as we only ever request settings for
        // 1 item
        let json_settings =
          (r.result as JsonArray)(0)? as JsonObject
        this._channel.log(
          "Received workspace/configuration " +
          "response = " + json_settings.string())
    ...
    // even if we have no valid settings, we still
    // call the compiler, as it has to be
    // initialized with apply_settings at least once
    this._compiler.apply_settings(maybe_settings)

  fun handle_register_capability_response(
    r: ResponseMessage val)
  =>
    // TODO: handle, check for error
    None

  fun handle_work_done_progress_create_response(
    r: ResponseMessage val)
  =>
    // TODO: handle, check for error
    None
  ...
  =>
    ...
    this._expect_responses_for.push(
  ...
  =>
    """
    Register to the LSP client for a certain
    capability/method.
    """
  ...
  be send_configuration_request() =>
    """
    Get all `pony-lsp`-scoped settings from the
    LSP client (editor).
    """
    this.send_request(
      Methods.workspace().configuration(),
