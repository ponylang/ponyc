...
class \nodoc\ _TestPathValidatorDriveLetter is UnitTest
  fun name(): String => "PathValidator/Windows drive letter"

  fun apply(h: TestHelper) =>
    h.assert_false(dep.PathValidator("C:foo"))
    h.assert_false(dep.PathValidator("c:foo"))
    h.assert_false(dep.PathValidator("D:bar/baz"))
