...
primitive _Num
  ...
  =>
    """
    Returns the offset of the last character of the integer
    in src at start_offset.
    """
    ...
    let predicate =
      if only_decimal then
        ...
        | '_' =>
          ...
          // as a hack we increase the offset past the underscore
          // to get a valid offset
          offset = offset + 1
          _ASCII~is_decimal()
        else
          _ASCII~is_decimal()
  ...
  fun float(src: String box, start_offset: USize): USize =>
    """
    Returns the offset of the last character of the float
    in src at start_offset.
    """
    // integer base 10
    ...
    if (c == U8('e')) or (c == U8('E')) then
      ...
      let exponent_end_offset =
        int(src, offset where only_decimal = true)
...
class \nodoc\ iso _FloatTest is UnitTest
