...
Positional args: workers rounds base-size ngap depth. Three more optional args --
floor span thresh -- set the arena knobs directly (bypassing the dial) for
re-tuning sweeps; omit them to honor --ponymemoryprofile.
"""
use "time"
...
actor Main
  new create(env: Env) =>
    let workers = try env.args(1)?.usize()? else 8 end
    let rounds = try env.args(2)?.usize()? else 2000 end
    let base = try env.args(3)?.usize()? else 65536 end
    let ngap = try env.args(4)?.usize()? else 8 end
    let depth = try env.args(5)?.usize()? else 32 end
