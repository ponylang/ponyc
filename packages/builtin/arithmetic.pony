...
trait val Real[A: Real[A] box] is
...
trait val Integer[A: Integer[A] box] is Real[A]
...
trait val _SignedInteger[A: _SignedInteger[A,C] box,
    C: _UnsignedInteger[C] box] is Integer[A]
  fun abs(): C
...
trait val _UnsignedInteger[A: _UnsignedInteger[A] box] is Integer[A]
  fun abs(): A
  fun string(fmt: FormatInt = FormatDefault,
    prefix: PrefixNumber = PrefixDefault, prec: U64 = -1, width: U64 = 0,
    align: Align = AlignRight, fill: U32 = ' '): String iso^
...
trait val FloatingPoint[A: FloatingPoint[A] box] is Real[A]
  fun tag epsilon(): A
  fun tag radix(): U8
