...
class Options ref is Iterator[_Result]
  ...
  fun ref _find_match(): _Match =>
    try
      let current = _args(_index)
      let pos = current.contains("=")
      let name: String val = current.substring(1, pos)
      ...
      | (None, var y: Option) => _args(_index) = current.cut_in_place(1, 1) ; y
      | (var x: Option, None) => x
      else
        error
      end
  ...
  fun ref next(): _Result =>
    if _skip_non_options() then
      ...
      | var m: Option => _env.stdout.print(m.token()) return ("Test", None)
      end
    end
