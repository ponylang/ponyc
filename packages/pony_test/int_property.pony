...
type IntUnitTest is PropertyTest[IntPropertySample]
  """
  Convenience alias for registering an `IntProperty` with PonyTest.
  """
...
trait IntProperty is Property[IntPropertySample]
  ...
  fun ref property(sample: IntPropertySample, h: TestHelper) ? =>
    let x = sample.int
...
type IntPairUnitTest is PropertyTest[IntPairPropertySample]
  """
  Convenience alias for registering an `IntPairProperty` with PonyTest.
  """

trait IntPairProperty is Property[IntPairPropertySample]
  """
  Tests a property across all 14 Pony integer types with two values per
  sample, dispatching each to `int_pair_property[T]`.
  """
  ...
  fun ref property(sample: IntPairPropertySample, h: TestHelper) ? =>
    let x = sample.int1
    let y = sample.int2
    match sample.choice % 14
