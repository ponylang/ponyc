...
* A `property` method that asserts a certain property for each sample
  ...
  [PropertyHelper](pony_check-PropertyHelper.md) which tries to expose a similar API
  as [TestHelper](pony_test-TestHelper.md).
* Optionally, the method ``params()`` can be used to configure how PonyCheck executes
  the property by specifying a custom [PropertyParams](pony_check-PropertyParams.md) object.
...
1. In order to pass your Property to the PonyTest engine, you need to wrap it inside a [Property1UnitTest](pony_check-Property1UnitTest.md).
...
2. Run as much [Properties](pony_check-Property1.md) as you wish inside one PonyTest
   [UnitTest](pony_test-UnitTest.md) using the convenience function
   [PonyCheck.for_all](pony_check-PonyCheck.md#for_all) providing a
   [Generator](pony_check-Generator), the [TestHelper](pony_test-TestHelper.md) and the
...
Independent of how you integrate with [PonyTest](pony_test--index.md),
