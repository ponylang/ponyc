...
class \nodoc\ _FAGracefulClose is AsioBackend
  """
  Like `_FABackpressureRetry` but exposes `main_event()` so the TCP mock
  can inject a readable on shutdown.
  """
  var _target: (_AsioEventInjectable tag | None) = None
  var _main_event: AsioEventID = AsioEvent.none()
  var _timer_event: AsioEventID = AsioEvent.none()
...
class \nodoc\ _FBGracefulClose is TCPBackend
  """
  `sendv` returns Retry on step 0 (triggers backpressure) then Ok on
  step 1+ (drains the queue). `shutdown` injects a readable event to
  simulate the peer's FIN response. `receive` returns Error after
  shutdown, completing the graceful close via hard_close.
  """
  var _send_step: USize = 0
  var _shutdown_called: Bool = false
  var _target: (_AsioEventInjectable tag | None) = None
  var _event: AsioEventID = AsioEvent.none()

  new create() => None

  fun ref set_close_target(
    target: _AsioEventInjectable tag,
    event: AsioEventID)
  =>
    _target = target
    _event = event
  ...
  fun ref shutdown(fd: U32) =>
    ...
    match _target
    | let t: _AsioEventInjectable tag =>
      t._inject_asio_event(_event, AsioEvent.read())
    end
...
class \nodoc\ iso _TestGracefulCloseWithPending is UnitTest
  """
  A graceful `close()` with data still queued under backpressure flushes
  those queued writes before shutting down. The accepted send fires
  `_on_sent` before `_on_closed`; `_on_send_failed` never fires.
  """
  fun name(): String => "net/mock/GracefulCloseWithPending"

  fun apply(h: TestHelper) =>
    h.expect_action("sent")
    h.expect_action("closed")

    let a = _TestGracefulCloseWithPendingActor(h)
    h.dispose_when_done(a)
    h.long_test(5_000_000_000)

actor \nodoc\ _TestGracefulCloseWithPendingActor
  is (TCPConnectionActor[_FBGracefulClose, _FAGracefulClose]
    & ServerLifecycleEventReceiver[_FBGracefulClose, _FAGracefulClose]
    & _AsioEventInjectable)
  var _tcp_connection:
    TCPConnection[_FBGracefulClose, _FAGracefulClose] =
    TCPConnection[_FBGracefulClose, _FAGracefulClose].none()
  let _h: TestHelper
  var _sent_first: Bool = false

  new create(h: TestHelper) =>
    _h = h
    _tcp_connection =
      TCPConnection[_FBGracefulClose, _FAGracefulClose].server(
        TCPServerAuth(_h.env.root),
        _SyntheticFd(),
        this,
        this)
  ...
  fun ref _on_started() =>
    _tcp_connection._tcp_ops().set_close_target(
      this, _tcp_connection._asio_ops().main_event())
  ...
  fun ref _on_closed() =>
    _h.assert_true(_sent_first, "_on_sent must precede _on_closed")
    _h.complete_action("closed")
