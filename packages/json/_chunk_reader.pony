...
class _ChunkReader
  ...
  Two cursors: the consume cursor (bytes before it are gone) and a single
  forward scan cursor used to find a string's extent before deciding
  whether to view or decode it. The scan cursor is a node reference, so
  it advances in O(1) and survives chunks being dropped behind it. Only
  one leaf scans at a time, so one scan cursor suffices; it is valid from
  `scan_reset()` until the next consume.
  ...
  fun ref scan_byte(): U8 ? =>
    """
    The byte at the scan cursor. Errors if the scan is at the end. Picks
    up chunks fed after a suspend: acquires the head if the scan was
    reset on an empty list, and hops onto the next chunk at a boundary.
    """
    if scan_at_end() then error end
    var node =
      match \exhaustive\ _scan_node
    ...
    if _scan_off >= node()?.size() then
      ...
      node =
        try node.next() as ListNode[Array[U8] val]
        else _Unreachable(); node
        end
      _scan_node = node
      _scan_off = 0
    end
  ...
  fun ref scan_advance() =>
    """
    Advance the scan cursor one byte (chunk hop happens in scan_byte).
    """
  ...
  fun view(): (String val | None) =>
    """
    A zero-copy view of [consume cursor, scan cursor) if it lies within
    one chunk, else `None`. The returned `String` shares the fed chunk's
    memory (the chunk's backing block stays live as long as the `String`
    is held).
    """
    try
      match \exhaustive\ _scan_node
