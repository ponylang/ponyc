...
Two boundaries to know about. Windows targets are not supported yet: a package containing a `.c` fails there with "C shims are not yet supported when targeting Windows" until MSVC header discovery lands — and that includes a dependency you pull in that ships a shim. And `--safe` does not gate shims: like `use "lib:..."` (which can already link an arbitrary prebuilt object), a dependency's shim is compiled and linked regardless of the `--safe` list, and its code can run at program start via C constructors. Shims add no capability that `use "lib:"` didn't already grant, but if you audit dependencies, `.c` files are now part of what you're auditing.
