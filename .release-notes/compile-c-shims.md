...
One platform boundary to know about: Windows targets are not supported yet. A package containing a `.c` fails there with "C shims are not yet supported when targeting Windows" until MSVC header discovery lands — and that includes a dependency you pull in that ships a shim.
