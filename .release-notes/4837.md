...
- **ponypath**: A string, containing a path or list of paths (like used e.g. in the `$PATH` envirobnebt variable) that will be used to tell `pony-lsp` about additional entries to its package search path.
...
`pony-lsp` only displayed the main error message of errors provided by ponyc. With this PR it is displaying also all additional informational items, that help explain the error.
