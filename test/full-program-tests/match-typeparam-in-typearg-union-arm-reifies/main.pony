...
//
// A type argument that is a union containing a type parameter used to
// be rejected at compile time with "this pattern can never match", even
// when the parameter could reify to make the union arms line up. Here
// A reifies to U8 so `Cell[(P | A)]` and `Cell[(P | U8)]` share a
...
actor Main
  new create(env: Env) =>
    let c: Cell[(P | U8)] = Cell[(P | U8)](U8(7))
